 ...
 *
 *      $Id: function_core.php 2014-07-29 10:20:00 sanree checkedby liuhuan $
 */
...
function sanree_brand_multi($num, $perpage, $curpage, $mpurl, $maxpages = 0, $page = 10, $autogoto = FALSE, $simple = FALSE, $jsfunc = FALSE) {
		...
		if(defined('IN_ADMINCP')) {
			...
			if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
				...
				$lang['prev'] = '<b></b>';//'&nbsp;&nbsp;';
				$lang['next'] = '<b></b>';//lang('core', 'nextpage');
			}
...
function syngroup($bid) {
	...
	foreach ($bids as $ibid) {
		...
		if ($lastdata['syngrouptid'] && $postgroup) {
			...
			$setarr['jointype'] =0; //0自由加入 2审核加入 1邀请加入
			$setarr['gviewperm'] = 1;//1所有人 0仅成员
...
function getcateurl($param,$zero = FALSE){
    ...
    if ($is_rewrite) {
	    $urllistmode = empty($config['urllistmode']) ? "brand-index-{tid}-{did}-{filter}-{listmode}.html": $config['urllistmode'];
...
function gettagurl($tagid) {
	...
	if ($config['is_rewrite']&&$config['urltagmode']) {
		$keylist = array('tid');
		$tid  = $tagid;
		$urltagmode = empty($config['urlitemmode']) ? "brand-tag-{tid}.html": $config['urltagmode'];
		foreach($keylist as $line) {
			$urlitemmode = str_replace("{".$line."}",$$line ,$urltagmode);
		}
		return branddomain().$urltagmode;
	}
	return branddomain().'plugin.php?id=sanree_brand&mod=tag&tid='.$tagid;
}
