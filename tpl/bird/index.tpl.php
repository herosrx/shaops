...
<script type="text/javascript" >
jQuery(document).ready(function(){
jQuery("#cardlist li").each(function(index){
jQuery(this).mouseover(function(){
    //清除没有访问过的层，同时计算当前的相对位置，实现等位置平移效果
    jQuery("div.box").css({display:"none"});
    var height = index * 32+"px";
	//显示具体内容效果
	jQuery("div.box").eq(index).addClass("show showcontent").css({display:"block",top:height});
	jQuery(this).addClass("hover");
}).mouseleave(function(){
	//关闭所有的层效果
	jQuery("div.box").eq(index).css({display:"none"});
	jQuery(this).removeClass("hover");
	});
});
jQuery(".box").hover(function(){
//获取当前选中的状态的时候让其处于显示状态
jQuery('.box').eq(jQuery('.box').index(jQuery(this))).addClass("show showcontent").css({display:"block"});
//同时添加背景效果
jQuery("#cardlist li").eq(jQuery('.box').index(jQuery(this))).addClass("hover");},function(){
	//移除当前效果，同时隐藏内容层
	jQuery("#cardlist li").removeClass("hover");
	jQuery('div.box').css({display:"none"});
}); 
});
</script>
    ...
    <div class="sr_nav">
      ...
      <div class="lb">
        <ul id="cardlist" class="l">
          ...
          <li {$cate[class]}> <a href="{$cate[url]}">
            <div class="cat l">{$cate[name]}</div>
            <div class="ar r">></div>
            </a>
		  </li>
        <!--{/loop}-->
        	...
        	<!--{loop $subcategory_bird $key $cate}-->
                ...
                <!--{else}-->
                    <div class="box">
                        <ul>
                        <!--{loop $cate $subcate}-->
                        	<li {$subcate[class]}><a href="{$subcate[url]}">{$subcate[name]}</a></li>
                        <!--{/loop}-->
                        </ul>
                    </div>
                <!--{/if}-->
      ...
      <div class="Cont">
        ...
        <div class="ScrCont" id="viewer">
          ...
          <!--{loop $newbrandlist $cate}-->
            <div><a target="_blank" href="{$cate[url]}" title="{$cate[name]}"><img src="$_G[siteurl]{BIRD_CSS}timthumb.php?src=$_G[siteurl]{$cate[poster]}&h=73&w=140&zc=1" /></a></div>
          <!--{/loop}-->
          </div>
        </div>
	...
	<!--{if $copyrightshow}-->
          <div class="copyright">
            <div class="brand">
              ...
              V&nbsp;&nbsp;{$pluginversion}&nbsp;&nbsp;For&nbsp;&nbsp;X2,&nbsp;X2.5,&nbsp;X3,&nbsp;X3.1,&nbsp;X3.2!&copy;<a href="http://dx.sanree.com/" target="_blank">Sanree.com</a></p>
              <div class="box_copyright"><a href="http://dx.sanree.com/" target="_blank"><b>{lang sanree_brand:h_sanree}</b> sanree.com</a></div>
            </div>
          </div>
          <!-- /copyright -->
        <!--{else}-->
